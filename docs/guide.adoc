...
The function block starts with `begin` and ends with `end`.
...
move_player :: fn player: Player, steps: int do
    player.pos[0] += player.vel[0] * steps
    player.pos[1] += player.vel[1] * steps
end
