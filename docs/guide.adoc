...
to create "method-like" behaviours when combined with `blob`-s.
...
----
Here we create a `Player`-blob, with a built in update-function that
mutates only one specific `Player`-blob. The `Player`-blob is "baked"
into the function and cannot be changed after creation.
