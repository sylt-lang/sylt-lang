...
The type of the variable is inferred but can be supplied.
...
The function block starts with `do` and ends with `end`. The `do` can be
replaced with a `->` if the function returns something. See the following
examples.
...
into the update-function as the constant `self`.
...
// Leaving this here until we decide what happens with ducktyping
