...
impl From<OwnedValue> for Value {
    fn from(value: OwnedValue) -> Self {
        match value {
            ...
            OwnedValue::Function(params, ty, slot) => Value::Function(Rc::new(params.into_iter().map(|param| Rc::new(RefCell::new(param.into()))).collect()), ty, slot),
...
impl From<&Value> for OwnedValue {
    fn from(value: &Value) -> Self {
        match value {
            ...
            Value::Function(params, ty, slot) => OwnedValue::Function(params.iter().map(|param| (&*param.borrow()).into()).collect(), ty.clone(), *slot),
